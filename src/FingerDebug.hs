...
module FingerDebug(mainF) where
...
getUser conn username = do
  ...
  case results of [] -> return $ Nothing
...
formatUser (User _ username shell homeDir realName _) = BS.concat ["Login: ", e username, "\t\t\t\t",
       "Name: ", e realName, "\n",
       "Directory: ", e homeDir, "\t\t\t",
       "Shell: ", e shell, "\n"]
    where e = encodeUtf8
...
returnUser dbConn soc username = do
  ...
  case maybeUser of Nothing -> do putStrLn ("Couldn't find matching user\ \ for username: " ++ (show username))
                                  return ()
